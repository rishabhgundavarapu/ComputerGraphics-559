 ...
 * @property {number} [size=1]
 */
export class NewRide extends GrObject{
    /**
   * @param {RideProperties} params
   */
  constructor(params={}){
    let width = 2 ; 
    let newride = new T.Group();
    let ride_geom = new T.CylinderGeometry(width,width,1,32)
    let base_mat = new T.MeshStandardMaterial({
      color: "red",
      metalness: 0.3,
      roughness: 0.8
    });
    let base = new T.Mesh(ride_geom, base_mat);
    base.translateY(0.1);
    base.scale.set(0.8,0.8,0.8)
    newride.add(base);
    
    let pole_geom =  new T.CylinderGeometry(1,1,6)
    let my_pole = new T.Mesh(pole_geom,base_mat)
    my_pole.position.y=-1.5
    my_pole.scale.set(0.5,0.5,0.5)
    newride.add(my_pole)




    super(`NewRide-${new_ride++}`, newride);
    this.whole_ob = newride;


    // put the object in its place
    this.whole_ob.position.x = params.x ? Number(params.x) : 1;
    this.whole_ob.position.y = params.y ? Number(params.y) : 3;
    this.whole_ob.position.z = params.z ? Number(params.z) : 1;
    let scale = params.size ? Number(params.size) : 1;
    newride.scale.set(scale, scale, scale);
    this.swing_angle = 0;
  }
  stepWorld(delta, timeOfDay,timestamp) {

    // in this animation, use the sine of the accumulated angle to set current rotation.
    // This means the swing moves faster as it reaches the bottom of a swing,
    // and faster at either end of the swing, like a pendulum should.
    this.swing_angle = 0.005 * delta;
    this.whole_ob.rotateY(0.5*this.swing_angle)
    let theta = timeOfDay* 1000;
    let x = 3 * Math.cos(theta);
    let z = 3 * Math.sin(theta);
    console.log(theta)
    this.whole_ob.position.x=x*4
...
export class GrCarousel extends GrObject {
  ...
  constructor(params = {}) {
    ...
    let horse_mat = new T.MeshStandardMaterial({color:'brown'})
    let horse_geom = new T.BoxGeometry(1,1,1)
    let my_horses = []
    ...
    for (let i = 0; i < num_poles; i++) {
      ...
      horse.position.y=Math.random()*1
      my_horses.push(horse)
      opole.add(horse)
   ...
   */
        stepWorld(delta, timeOfDay) {
          ...
          this.whole_ob.rotateY(0.5*this.swing_angle)
          for(let i =0;i<=7;i++){
            this.my_horses[i].position.y += direction * 0.01
            console.log(this.my_horses[i].position.y)
            if(this.my_horses[i].position.y >=0.95 && direction == 1){
                  direction *= -1
                  console.log(this.my_horses[i].position.y = Math.sin(delta))
            }
          else if(this.my_horses[i].position.y < 0.05 && direction ==-1){
            direction *= -1
          }}
        }
