...
// A wrecking ball used to demolish buildings
let wreckingball = 0;
 ...
 * @property {number} [size=1]
 */
export class GrWBall extends GrObject {
  /**
   * @param {WreckingBallProperties} params
   */
  constructor(params = {}) {
    let wball = new T.Group();
    ...
    let ball_geom = new T.SphereGeometry(1)
    let ball_mat = new T.MeshStandardMaterial({color:'green',metalness:1})
    ...
    // so if your starting value does not fall on one of the 30 steps,
    // the starting value in the UI may be slightly different from the starting value you gave.
    super(`WreckingBall-${wreckingball++}`, wball, [
      ["x", -4, 4, 0],
      ["z", -4, 4, 0],
      ["theta", 0, 360, 0],
      ["wire", 2, 3.5, 1.5],
    ...
    this.whole_ob.position.z = params.z ? Number(params.z) : 0;
    let scale = params.size ? Number(params.size) : 1;
    wball.scale.set(scale, scale, scale);
  }
  ...
  update(paramValues) {
    ...
    this.wire.position.x = paramValues[3];
    this.arm.rotation.y = degreesToRadians(paramValues[4]);
    console.log(paramValues[5])
    this.ball.scale.set(paramValues[5],paramValues[5],paramValues[5])
...
export class GrForkLift extends GrObject {
  ...
  constructor(params = {}) {
    ...
    pedestal_curve.lineTo(-0.35, 0);
    let pedestal_geom = new T.ExtrudeGeometry(pedestal_curve, exSettings);
    let new_mat = new T.MeshStandardMaterial({color:'red'})
    let pedestal = new T.Mesh(pedestal_geom, new_mat);
    forklift.add(pedestal);
    pedestal.translateY(0.6);
    pedestal.translateZ(-0.2);
    ...
    cab_curve.lineTo(-1, 0);
    let cab_geom = new T.ExtrudeGeometry(cab_curve, exSettings);
    let cab = new T.Mesh(cab_geom, new_mat);
    cab_group.add(cab);
    cab.translateZ(-0.2);
    //forks
    let forkgrp = new T.Group();
    let forkbgeom = new T.BoxGeometry(.2, 1, 2);  
    let forkBaseMesh = new T.Mesh(forkbgeom, new_mat);
    forkgrp.add(forkBaseMesh);
    let forkageom = new T.BoxGeometry(1.6, .05, .4);  
    let fork1 = new T.Mesh(forkageom, new_mat);
    fork1.position.x = -.8;
    fork1.position.z = .6;
    let fork2 = new T.Mesh(forkageom, new_mat);
    fork2.position.x = -.8;
    fork2.position.z = -.6;
    forkgrp.add(fork1);
    forkgrp.add(fork2);
    forkBaseMesh.position.y = .42;
    forkgrp.position.x = -1.2;
    forkgrp.position.z = 0;
    forkgrp.position.y=1
    forklift.add(forkgrp)
    cab_group.add(forkgrp)
    // Next up is the first part of the bucket arm.
    // In general, each piece is just a series of line segments,
    // plus a bit of extra to get the geometry built and put into a group.
    // We always treat the group as the "pivot point" around which the object should rotate.
    ...
    // super and we have to call super before we can use this
    // The parameters for sliders are also defined here.
    super(`Forklift-${forkliftobj++}`, forklift, [
      ["x", -10, 10, 0],
      ["z", -10, 10, 0],
      ["theta", 0, 360, 0],
      ["spin", 0, 360, 0],
  ...
  update(paramValues) {
    ...
    this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
    this.cab.rotation.y = degreesToRadians(paramValues[3]);
    this.fk.position.y = paramValues[4]
  }
}